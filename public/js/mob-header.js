...
@media(max-width:1023px){
  ...
  /* ── Account link: staff → /admin, client → /profile, guest → /login ── */
  const acctHref = localStorage.getItem('rh_token') ? '/admin'
    : sessionStorage.getItem('rh_profile_token') ? '/profile'
    : '/login';

  const ACCT_SVG = `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="8" r="4"/><path d="M4 20c0-4 3.6-7 8-7s8 3 8 7"/></svg>`;
  ...
  top.innerHTML = `
    ...
    <a href="${acctHref}" class="rh-mob-top-btn" title="Кабинет">${ACCT_SVG}</a>`;
